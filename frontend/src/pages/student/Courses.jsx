...
export default function StudentCourses() {
  ...
  const [search, setSearch] = useState(() => {
    const saved = localStorage.getItem("studentCourses_search");
    return saved || "";
  });
  const [level, setLevel] = useState(() => {
    const saved = localStorage.getItem("studentCourses_level");
    return saved || "";
  });

  // Save filters to localStorage whenever they change
  useEffect(() => {
    localStorage.setItem("studentCourses_search", search);
  }, [search]);

  useEffect(() => {
    localStorage.setItem("studentCourses_level", level);
  }, [level]);
