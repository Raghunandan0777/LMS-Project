...
export default function AdminUsers() {
  ...
  const handleDelete = async (id, name) => {
    ...
    try { await adminAPI.deleteUser(id); toast.success("User deleted"); fetch(); }
  ...
  return (
    <div>
      <div className="page-header">
        <div><h1 className="page-title">User Management</h1><p className="page-subtitle">{total} total users</p></div>
      </div>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap" }}>
        <div style={{ position: "relative", flex: 1, minWidth: 200 }}>
          ...
          <input className="form-input" style={{ paddingLeft: 38 }} placeholder="Search by name or email..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
              ...
              <tbody>
                {filtered.map((u) => (
                  <tr key={u._id}>
                    ...
                    <td>
                      {u.role === "instructor" ? (
                        <span className={`badge badge-${u.isApproved ? "green" : "amber"}`}>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={6} style={{ textAlign: "center", padding: 40, color: "var(--text3)" }}>No users found</td></tr>
                )}
              </tbody>
            </table>
          </div>
